...
async function scrapeQuestions(page, numPages) {
  ...
  for (let i = 0; i < numPages; i++) {
    ...
    // ③-2 問題画像のURLがある場合は、ページ上の対象画像要素をキャプチャーして Base64 に変換
    const processedProblemImages = [];
    for (const src of problemImageSrcs) {
      if (src.startsWith('http')) {
        try {
          // ページ内から対象の img 要素を取得（セレクタは src 属性で特定）
    ...
    const problemData = {
      ...
      problemImageSrcs: processedProblemImages, // ここで変換済みの画像データをセット
      problemId: problemId,
      choices: choices
    };
    ...
    // ⑦-2 解説画像についても、URLの場合はキャプチャーして Base64 に変換
...
async function processImage(src) {
  if (src.startsWith('data:')) {
    try {
      // data URL からBase64部分を取り出し、Bufferに変換
 ...
 *   explanation: { correctAnswer, explanationPoints, optionAnalysis, guideline, explanationImageSrcs }
 * }
 *
 * ※ 各画像は、A4用紙の利用可能横幅（左右マージン除く、ここでは515.28ptと仮定）を 3 分割した幅以内に収まるように縮小します。
 */
async function generatePdf(contents, fileName) {
  ...
  // A4用紙の左右マージン（例：40ptずつ）を除いた利用可能横幅
  const availableWidth = 515.28;
  // 各画像は、A4横幅の3分割幅以内に収める
  const maxAllowedWidth = availableWidth / 3; // 約171.76pt

  // ヘルパー関数：画像オブジェクト obj { dataUrl, dimensions } を受け取り、
  // 自然な幅（dimensions.width）が maxAllowedWidth を超えていれば maxAllowedWidth に縮小し、
  // そうでなければそのままの幅を返します。
  ...
  for (const content of contents) {
    ...
    if (content.problem.problemImageSrcs && content.problem.problemImageSrcs.length > 0) {
      ...
      if (total === 1) {
        // 1枚の場合は、単体で表示。幅は縮小して maxAllowedWidth 以下に収める
        ...
        // 各行では、各セルの幅は (availableWidth ÷ (その行の画像数)) ではなく
        // 各画像は maxAllowedWidth 以下になるように縮小するため、各セルの指定は getScaledWidth(obj)
        let tableBody = [];
        // 1行目
        let row1 = [];
